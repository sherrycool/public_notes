...
- 基本原理
  ...
  线性判别：先将样本点投影到一维空间，若效果不明显，可以考虑增加一个维度，即投影到二维空间，以此类推。二次判别到适用性比现行判别函数要广。

  贝叶斯判别：用先验概率P(A|B)利用贝叶斯公式，求出后验概率P(B|A)，该样本属于某一类的概率，然后选择具有最大后验概率的类作为该样本所属的类。优势在于，不怕噪声和无关变量，不足是假设各特征属性是无关的，但是现实是各属性间往往并不独立，是有较强相关性的。

  距离判别：根据待判定样本与已知样本之间的距离远近作出判别。根据已知类别样本信息建立距离判别函数式，再将各判定样本的属性数据逐一带入计算，获得距离值，距离值最小的类别样本选入。

  K最紧邻方法，主要依靠周围有限近邻样本的信息，不是靠判别类域的方法来确定样本，不使用类域有交叉活重叠较多的待分样本。

  有权重的K最近邻算法：加入权重，效果更佳。
...
- 用途：解决小样本、非线性及高纬度模式识别。并且能够推广到函数拟合等其他机器学习问题中。

  小样本：传统统计学研究等是样本无穷大的渐进理论。

- 基本原理

  支持向量机：建立在统计学理论的VC维理论和结构风险最小原理基础上，根据有限样本在模型等复杂性和学习能力之间寻求最佳折中，目的是获得最好的推广能力。
